...
class Settings extends Authorization
{
    ...
    function profile()
    {
        ...
        } elseif ($this->session->userdata('user_role') == "cook") {
            $page_data['user_info'] = $this->cook_model->get_by_id($this->session->userdata('user_id'));
        }
        $this->load->view('backend/index', $page_data);
    }
    ...
    // order function responsible for showing the delivery settings page.
    function order()
    {
        authorization(['admin'], true);
        $page_data['page_name'] = 'settings/order';
        $page_data['page_title'] = get_phrase("order_settings");
        $this->load->view('backend/index', $page_data);
    }
