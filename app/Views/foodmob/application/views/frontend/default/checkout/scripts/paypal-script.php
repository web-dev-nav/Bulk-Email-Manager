  ...
  paypal.Button.render({
    ...
    payment: function(data, actions) {
      return actions.payment.create({
        payment: {
          transactions: [{
            amount: {
              total: '<?php echo sanitize($amount_to_pay); ?>',
              currency: '<?php echo sanitize($paypal[0]->currency); ?>'
            }
          }]
        }
      });
    ...
    onAuthorize: function(data, actions) {
      ...
      return actions.payment.execute().then(function() {
        ...
        var redirectUrl = '<?php echo site_url('checkout/paypal_payment/' . $order_type . '/' . $amount_to_pay . '/' . $address_number); ?>' + '/' + data.paymentID + '/' + data.paymentToken + '/' + data.payerID;
